...
function ScoreCard({ match }) {
    ...
    const batting = (activeInnings === 'home' ? homeInnings : awayInnings).Batsmen || [];
    const extras = {
        byes: Number((activeInnings === 'home' ? homeInnings : awayInnings).Byes || 0),
        legByes: Number((activeInnings === 'home' ? homeInnings : awayInnings).Legbyes || 0),
        wides: Number((activeInnings === 'home' ? homeInnings : awayInnings).Wides || 0),
        noBalls: Number((activeInnings === 'home' ? homeInnings : awayInnings).Noballs || 0),
        penalty: Number((activeInnings === 'home' ? homeInnings : awayInnings).Penalty || 0)
    };
    extras.total = Object.values(extras).reduce((sum, v) => sum + v, 0);

    const fallOfWickets = (activeInnings === 'home' ? homeInnings : awayInnings).FallofWickets || [];
    const partnerships = (activeInnings === 'home' ? homeInnings : awayInnings).Partnerships || [];
    ...
    const getBowlerName = (bowlerId) => {
        const bowlerPlayer = Object.values(teams[activeInnings === 'home' ? awayId : homeId].Players).find(player => player.Position === bowlerId);
        return bowlerPlayer?.Name_Full || 'Unknown Bowler';
    };

    const didNotBat = batting.filter(p => !p.Howout);

    const oppositeTeamName = activeInnings === 'home' ? awayTeamName : homeTeamName;
            ...
            <div className='section-width'>
                <div className="bg-[url('/images/scorecard/bgcardmain.svg')] bg-cover text-white mt-10 sm:mt-20 rounded-lg">
                    ...
                    <div className='flex justify-center pb-2'>
                        <span className="text-sm font-medium uppercase bg-[#2D3F87] text-center py-1 rounded-lg px-2">{match.Matchdetail.Equation}</span>
                    </div>
                </div>

                {/* Tabs for switching innings */}
                <div className="flex justify-center mt-10 bg-transparent rounded-t-lg bg-gradient-to-r from-[#203376] via-black to-[#203376]">
                    <button
                        onClick={() => switchTab('home')}
                        className={`flex-1 px-4 py-2 text-center ${activeInnings === 'home' ? 'text-orange-500 border-b-4 border-orange-500' : 'bg-transparent text-white'}`}
                    >
                        {homeTeamName}
                    </button>
                    <button
                        onClick={() => switchTab('away')}
                        className={`flex-1 px-4 py-2 text-center ${activeInnings === 'away' ? 'text-orange-500 border-b-4 border-orange-500' : 'bg-transparent text-white'}`}
                    >
                        {awayTeamName}
                    </button>
                </div>
                ...
                {/* Scorecard */}
                <div className="bg-[#0F1A2D] overflow-hidden shadow-md mb-8">
                    <div className="flex items-center bg-[#0F1A2D] text-white px-6 py-4">
                        <div className="flex items-center space-x-3 flex-1">
                            <Image
                                src={activeInnings === 'home' ? `/images/scorecard/${homeTeamName}.svg` : `/images/scorecard/${awayTeamName}.svg`}
                                alt={activeInnings === 'home' ? homeTeamName : awayTeamName}
                                width={100}
                                height={100}
                            />
                            <span className="uppercase font-medium">{activeInnings === 'home' ? homeTeamName : awayTeamName}</span>
                        </div>
                        <div className="text-sm">
                            <span className="font-semibold">
                                {(activeInnings === 'home' ? homeInnings : awayInnings).Total}/{(activeInnings === 'home' ? homeInnings : awayInnings).Wickets}
                            </span>
                            <span className="ml-2">({(activeInnings === 'home' ? homeInnings : awayInnings).Overs} Overs)</span>
                        </div>
                    </div>

                    {/* Batting Table */}
                    <div className="overflow-x-auto bg-[#0F1A2D] text-white px-6 py-4">
                        ...
                        <table className="w-full text-left text-sm border-collapse">
                            ...
                            <tbody>
                                {batting.filter(p => p.Howout).map((p, i) => (
                                    <tr key={i} className="border-b border-gray-800">
                                        <td className="py-2">{teams[activeInnings === 'home' ? homeId : awayId].Players[p.Batsman]?.Name_Full}</td>
                    ...
                    {didNotBat.length > 0 && (
                        <div className="bg-[#0F1A2D] text-white px-6 py-4 border-t border-gray-700">
                            ...
                            <ul className="text-sm">
                                {didNotBat.map((p, i) => (
                                    <li key={i} className="py-2">{teams[activeInnings === 'home' ? homeId : awayId].Players[p.Batsman]?.Name_Full}</li>
                                ))}
                            </ul>
                        </div>
                    )}
                        ...
                        <div className="bg-[#E5EBFF] px-4 rounded-lg pb-5">
                            <div className='flex items-center'>
                                <Image
                                    ...
                                    className='my-5'
                                />
                                <h3 className="text-2xl font-semibold text-black uppercase">Fall of Wickets</h3>
                            </div>
                            <div className="text-black">
                                {fallOfWickets.map((wicket, index) => (
                                    <span key={index}>
                                        {wicket.Score} ({teams[activeInnings === 'home' ? homeId : awayId].Players[wicket.Batsman]?.Name_Full}, {wicket.Overs} Overs)
                                    </span>
                                ))}
                            </div>
                        </div>
                ...
                {/* Bowling Stats */}
                <div className="bg-blue-900 text-white rounded-lg mb-10">
                    <div className="flex items-center bg-[url('/images/scorecard/bgcard.svg')] rounded-t-lg">
                        <Image
                            src={activeInnings === 'home' ? `/images/scorecard/${awayTeamName}.svg` : `/images/scorecard/${homeTeamName}.svg`}
                            alt={activeInnings === 'home' ? homeTeamName : awayTeamName}
                            width={100}
                            height={100}
                            className='my-5'
                        />
                        <h3 className="text-xl font-semibold mb-4">{oppositeTeamName}</h3>
                    </div>
                    <div className="overflow-x-auto px-3 sm:px-6 bg-[#0F1A2D] rounded-b-lg">
                        <table className="table-fixed w-full">
                            ...
                            <tbody>
                                {(activeInnings === 'home' ? homeInnings : awayInnings).Bowlers?.map((bowler, index) => (
                                    <tr key={index}>
                                        <td className="py-2 px-4 text-center">{teams[activeInnings === 'home' ? awayId : homeId].Players[bowler.Bowler]?.Name_Full}</td>
                                        ...
                                        <td className="py-2 px-4 text-center">{bowler.Noballs}</td>
                                        <td className="py-2 px-4 text-center">{bowler.Wides}</td>
                                        <td className="py-2 px-4 text-center">{bowler.Economyrate}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
